...
class kerasmodelwrapper(BaseEstimator,TransformerMixin):
  def __init__(self):
    ...
    self.model.load_weights("mobilenetv2_clean.weights.h5")

  def fit(self,X,y=None):
    return self

  def transform(self,X):
    # X is already preprocessed image data (NumPy array) from the previous step
    preds = self.model.predict(X)
    # Decode the predictions and return the top result
